...
const getInvoiceBuyMaterial = (filters) => {
  const paramsString = queryString.stringify(filters);
  return axios.get(
    INVOICES_API_URL_1 + `/list-invoices-material?${paramsString}`,
    filters
  );
};
...
const listInvoiceNoFixer = (filters) => {
  const paramsString = queryString.stringify(filters);
  return axios.get(
    INVOICES_API_URL_2 + `/list-invoices-no-fixer?${paramsString}`,
    filters
  );
};
...
const getInvoiceCanDelete = (filters) => {
  const paramsString = queryString.stringify(filters);
  return axios.get(
    INVOICES_API_URL_2 + `/list-can-delete?${paramsString}`,
    filters
  );
};
...
const checkMaterial = (checkQuantity) => {
  ...
  return axios.get(
    INVOICES_API_URL_2 + `/comparison?${paramsString}`,
    checkQuantity
  );
};
