...
export default function FixerInvoice(props) {
  ...
  const colorStatusInvoice = (status, confirm) => {
    if (status.localeCompare("Đang sửa") == 0 && confirm === 1) {
      return (
        <span
          style={{
            color: "rgb(255, 174, 6)",
  ...
  const handleChange = (id, status, confirm) => {
    changeStatus(id, status, confirm);
    editInvoice(id);
  };

  const changeStatus = (id, status, confirm, noteInvoice) => {
    InvoicesService.finish(id, status, confirm, noteInvoice)
      .then((res) => {
        ...
        <TableRow sx={{ "& > *": { borderBottom: "unset" } }}>
          ...
          <TableCell>
            <Button
              ...
              onClick={() => handleChange(row.id, 3, 2)}
            >
              Tiến hành sửa
            </Button>
          </TableCell>
  ...
  if (error) {
    ...
    return <Typography variant="h4">Chưa có đơn sửa</Typography>;
  } else {
    return (
      <>
        <Modal
                  ...
                  <Paper sx={{ width: "100%", mb: 2, minHeight: "100px" }}>
                    <TableContainer>
                      ...
                      >
                        ...
                        <TableBody>
                          <Row key={invoices.code} row={invoices} />
                        </TableBody>
                      </Table>
                    </TableContainer>
                  </Paper>
