...
export default function StatisticsAdmin() {
  ...
  const getData = async () => {
    ...
    const res1 = await axios({
      method: "get",
      url:
        "http://localhost:8080/api/admins/statistics/invoices?&dateStart=" +
        start +
        "&dateEnd=" +
        end,
    });
    ...
    const res6 = await axios({
      method: "get",
      url:
        "http://localhost:8080/api/admins/statistics/input?dateStart=" +
        start +
        "&dateEnd=" +
        end,
    });
    ...
    const res2 = await axios({
      method: "get",
      url:
        "http://localhost:8080/api/admins/statistics/services?dateStart=" +
        start +
        "&dateEnd=" +
        end,
    });
    ...
    const res3 = await axios({
      method: "get",
      url:
        "http://localhost:8080/api/admins/statistics/materials?dateStart=" +
        start +
        "&dateEnd=" +
        end,
    });
    ...
    const res4 = await axios({
      method: "get",
      url:
        "http://localhost:8080/api/admins/statistics/customers?&dateStart=" +
        start +
        "&dateEnd=" +
        end,
    });
    ...
    const resNumber = await axios({
      method: "get",
      url:
        "http://localhost:8080/api/admins/statistics/number?dateStart=" +
        start +
        "&dateEnd=" +
        end,
    });
    ...
    const resStore = await axios({
      method: "get",
      url:
        "http://localhost:8080/api/admins/statistics/store?dateStart=" +
        start +
        "&dateEnd=" +
        end,
    });
        ...
        <GridItem xs={12} sm={12} md={12}>
          <Card>
            ...
            <h4 className={classes.cardTitle}>
              Doanh thu theo ngày asdaasdasd
            </h4>
          ...
          <Card style={{ marginTop: "0px", height: "336px" }}>
            <CardHeader>
              <Typography variant="h5" align="center">
                Biểu đồ thống kê doanh thu theo từng cửa hàng
              </Typography>
            </CardHeader>
            <CardBody>
              <Bar
                data={{
                  labels: store.map((x) => x.name),
                  datasets: [
                    {
                      label: "Tổng doanh thu(vnđ)",
                      backgroundColor: ["#3e95cd", "#8e5ea2", "#3cba9f"],
                      data: store.map((X) => X.pirce),
                    },
                  ],
                }}
                height="270px"
