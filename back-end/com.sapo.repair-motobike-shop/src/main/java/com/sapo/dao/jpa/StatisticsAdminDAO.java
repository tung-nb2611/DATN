...
@Repository(value = "StatisticsAdminDAO")
@Transactional(rollbackOn = Exception.class)
public class StatisticsAdminDAO {
    @Autowired
    ...
    public List<InvoiceTotalDTO> selecInvoicesTotal(long dateStart, long dateEnd) {
        String sql = "select\n" +
                "       sum(tbl_invoices.total) as totalPrice,\n" +
                "       count(tbl_invoices.id) as countInvoice\n" +
                "from tbl_invoices\n" +
                "where tbl_invoices.store_id =" +"AND tbl_invoices.area_id="+" AND tbl_invoices.created_at between \n" + dateStart+
                "\n"+"And\n"+ dateEnd;
    ...
    public BigDecimal getSumInbvoice(int store_id,long dateStart, long dateEnd) {
        return jdbc.queryForObject("SELECT  sum(tbl_invoices.total) as totalPrice FROM  tbl_invoices WHERE tbl_invoices.store_id =?  and   tbl_invoices.end_at >=? and tbl_invoices.end_at <=? and status=5", new Object[]{store_id,dateStart,dateEnd}, BigDecimal.class);
    }
    public Integer getCountInbvoice(int store_id,long dateStart, long dateEnd) {
        return jdbc.queryForObject("SELECT  count(tbl_invoices.id) as countInvoice FROM  tbl_invoices WHERE tbl_invoices.store_id =? and    tbl_invoices.end_at >=? and tbl_invoices.end_at <=? and status=5", new Object[]{store_id,dateStart,dateEnd}, Integer.class);
    }
    ...
    public List<StatisticServiceDTO> selectServiveAndInvoiceInfo(long dateStart, long dateEnd) {
    ...
    //Hàm thống kê doanh thu
    public BigDecimal valueInvoice(long dateStart, long dateEnd) {
            return jdbc.queryForObject("SELECT  sum(tbl_invoices.total) as totalPrice FROM  tbl_invoices WHERE  tbl_invoices.end_at >=? and tbl_invoices.end_at <=? and tbl_invoices.status=5 ", new Object[]{dateStart,dateEnd}, BigDecimal.class);
        }
    //Hàm thống kê số lượng
    public Integer  countInvoice(long dateStart, long dateEnd) {
        return jdbc.queryForObject("SELECT  count(tbl_invoices.id) as countInvoice FROM  tbl_invoices WHERE   tbl_invoices.end_at >=? and tbl_invoices.end_at <=? and tbl_invoices.status=5", new Object[]{dateStart,dateEnd}, Integer.class);
    }
    //Hàm thống kê số đơn nhập  hàng
    public Integer  countRecipt(long dateStart, long dateEnd) {
        return jdbc.queryForObject("SELECT  count(tbl_receipts.id) FROM  tbl_receipts WHERE tbl_receipts.created_at >=? and tbl_receipts.created_at <=?  and tbl_receipts.type = 1", new Object[]{dateStart,dateEnd}, Integer.class);
    }
    //Hàm thống kê số đơn xuất hàng
    public Integer  countPayment(long dateStart, long dateEnd) {
        return jdbc.queryForObject("SELECT  count(tbl_receipts.id) FROM  tbl_receipts WHERE tbl_receipts.created_at >=? and tbl_receipts.created_at <=? and tbl_receipts.type = 2 ", new Object[]{dateStart,dateEnd}, Integer.class);
    }
    ...
    public  List<Invoice> findAllInvoiceByDate(long dateStart, long dateEnd){
        String sql = "SELECT * FROM tbl_invoices " +
                " Where   tbl_invoices.end_at >= " + dateStart +
                " AND tbl_invoices.end_at <= " + dateEnd;
        System.out.println(sql);
